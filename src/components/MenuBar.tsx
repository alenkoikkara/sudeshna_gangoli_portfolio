    ...
    <div className="w-full h-7 backdrop-blur-md flex items-center px-4 justify-between text-xs font-medium text-foreground z-50 relative">
      <div className="flex items-center gap-6">
        ...
        <div className="relative group flex items-center h-full">
          ...
          <div className="absolute top-full left-0 pt-3 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] translate-y-[-4px] group-hover:translate-y-0 scale-95 group-hover:scale-100 origin-top-left z-50">
            <div className="relative bg-white/20 dark:bg-black/30 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.25)] rounded-2xl p-2 min-w-[140px]">
              {/* Border Mask */}
              <div 
                className="absolute inset-0 rounded-2xl border border-white/40 dark:border-white/20 pointer-events-none"
        ...
        <div className="relative group flex items-center h-full">
          ...
          <div className="absolute top-full left-0 pt-3 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] translate-y-[-4px] group-hover:translate-y-0 scale-95 group-hover:scale-100 origin-top-left z-50">
            <div className="relative bg-white/20 dark:bg-black/30 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.25)] rounded-2xl p-2 min-w-[200px]">
              {/* Border Mask */}
              <div 
                className="absolute inset-0 rounded-2xl border border-white/40 dark:border-white/20 pointer-events-none"
                style={{ clipPath: 'polygon(0% 100%, 100% 100%, 100% 0%, calc(24px + 10px) 0%, calc(24px + 10px) 4px, calc(24px - 10px) 4px, calc(24px - 10px) 0%, 0% 0%)' }}
              ...
              <div className="relative z-10 flex flex-col gap-1 w-full">
                <button
                  onClick={() => {
                    ...
                    if (el) {
                      el.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>';
